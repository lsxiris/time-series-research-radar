# Characteristic Root Analysis and Regularization for Linear Time Series Forecasting — OpenReview Structured Summary

- OpenReview ID: `JTtwGRACte`
- Forum: https://openreview.net/forum?id=JTtwGRACte
- PDF: https://openreview.net/pdf?id=JTtwGRACte
- Venue status: ICLR 2026 Poster
- 平均review分数（公开review）: 6.0
- 分数范围: 4.0-8.0

## reviewer concerns
- assumption/boundary: 2
- experimental adequacy: 1
- theory/identifiability: 1
- robustness/hyperparameter: 1
- innovation: 1
- complexity/efficiency: 1

## author responses
- 补实验/补指标: 1
- 补解释/补理论: 1
- 补相关工作: 1
- 收缩主张/承认局限: 1
- 补效率报告: 1
- 改表述/改图示: 1

## decision / meta review
- Paper Decision
- Accept (Poster)

## global response highlights


## score change
- 暂未自动抓到显式 score change 记录；当前保留公开评审分数区间 `4.0-8.0` 作为 rebuttal 前窗口。
